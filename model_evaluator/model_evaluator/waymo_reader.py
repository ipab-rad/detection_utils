...
class WaymoDatasetReader2D(DatasetReader2D):
    ...
    def decode_image(image_component: v2.CameraImageComponent) -> np.ndarray:
        return cv2.imdecode(np.frombuffer(image_component.image, dtype=np.uint8), cv2.IMREAD_COLOR)
    ...
    @staticmethod
    def decode_detections(box_component: v2.CameraBoxComponent) -> list[Detection2D]:
        detections = []

        for cx, cy, w, h, type in zip(box_component.box.center.x,
                                      box_component.box.center.y,
                                      box_component.box.size.x,
                                      box_component.box.size.y,
                                      box_component.type):
            bbox = BBox2D.from_cxcywh(cx, cy, w, h)
            label = WaymoDatasetReader2D.decode_label(type)
    ...
    def read_data(self) -> list[tuple[np.ndarray, list[Detection2D]]]:
        ...
        for context_name in context_names:
            ...
            for timestamp in context_names[context_name]:
                frame_df = image_w_box_df.loc[image_w_box_df['key.frame_timestamp_micros'] == timestamp]
