...
class RosbagReader:
    def __init__(
            ...
            if topic_info.topic_metadata.name in self.topics_with_types.keys():
                try:
                    ...
                    if (
                        ...
                        raise TypeError(
                            f"Topic '{topic_info.topic_metadata.name}' has different type '{topic_type}' ('{topic_info.topic_metadata.type}') than desired type '{self.topics_with_types[topic_info.topic_metadata.name]}'."
                        )

                    if print_info:
                        print(
                            f"Topic '{topic_info.topic_metadata.name}' ('{topic_type}'): {topic_info.message_count} messages ({topic_info.message_count / metadata.duration.total_seconds():.2f} Hz)"
                        )
        ...
        self.reader.set_filter(
            rosbag2_py.StorageFilter(topics=list(topics_with_type.keys()))
        )
