...
def analyse():
    ...
    chosen_range_files = [
        rf for n in chosen_range_names for rf in range_groups[n]
    ]
    chosen_vru_type_files = [
        vtf for n in chosen_vru_type_names for vtf in vru_type_groups[n]
    ]
    ...
    files_to_combine = [
        results_files[x]
        for x in range(len(results_files))
        if x in chosen_range_files or x in chosen_vru_type_files
    ]
    ...
    all_results = {
        label.name: {"gt_count": 0, "results": []} for label in labels_to_use
    }
    ...
    for label in labels_to_use:
        ...
        if gt_count == 0 or pred_count == 0:
            print(
                f"Cannot calculate AP for label {label.name} {gt_count=} {pred_count=}"
            )
            continue
        ...
        if label.name == curve_label:
            precisions, recalls = calculate_precisions_recalls(
                tp, fp, gt_count
            )
